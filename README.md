...
`Obsidian Image Manager` 是一个面向 Obsidian 的图片管理插件，`v3.1.0` 聚焦于够用实用的图片工作流、双语设置页与更克制清晰的发布展示面。
...
8. 如果重建后 Obsidian 仍显示旧行为，重新复制 `manifest.json`、`main.js`、`styles.css` 到插件目录并重载插件。
9. 运行批量压缩命令，确认暂停、恢复与取消行为正常。
10. 打开笔记画廊和文件夹画廊，确认筛选、排序与网格/列表切换可用。
11. 在 Markdown 笔记中运行当前文件转换命令，确认每张引用图片只被转换一次。
12. 运行任一整库命令，确认会先弹出风险确认框。
13. 连续执行多次图片修改后，验证 `撤销` 与 `重做` 命令可以来回切换最近几次操作。
14. 开启 `删除孤立图片` 后执行 `更新图片链接与目录`，确认只会删除当前受管目录中未被任何笔记引用的图片。
