...
- 修复 Live Preview 中点击图片链接行打开画廊的判定：只要点击发生在包含具体图片 embed 的同一行，就打开对应图片画廊。
- 支持选中图片后展开的 Markdown 图片语法、图片占位区、图片容器与真实图片本体等点击入口。
- 保留空白行、非图片 widget 行和源码模式 Markdown 图片链接的默认行为，避免文档底部空白误开画廊。
- 补充 Live Preview 图片链接行、选中图片语法、空白行和非图片 widget 的回归测试。
- 同步更新 `manifest.json`、`package.json`、`versions.json`、README、任务状态与网站版本文案，准备以 `4.0.12` 发布。

## 4.0.11

- 新增“空目录清理”能力：文件列表右键文件夹或 Markdown 文件时可清理关联空受控目录，命令面板保留整库清理并继续显示风险确认。
- 空目录清理会处理已删除或改名笔记遗留的空受控目录，不再只依赖现存 Markdown 文件反推候选目录。
- 补充文件列表右键、整库命令、遗留空目录和显式清理绕过自动开关的回归测试。
- 同步更新 `manifest.json`、`package.json`、`versions.json`、README、任务状态与网站版本文案，准备以 `4.0.11` 发布。

## 4.0.10

- 修复删除 md 后按笔记名创建的受控目录不会跟随清理的问题；删除事件只记录受控目录目标，等待 Obsidian 元数据稳定后再按目录收敛。
- 批量删除多个 md 时按受控目录去重，并把本批次删除的笔记路径作为清理范围，避免交叉引用图片导致漏删或误删。
- 元数据等待超时时只删除真实空目录，不删除图片；固定公共目录、同级共享目录和留空保存位置不会因删除单篇 md 被自动扫描。
- 增强恢复事务对 stale 文件对象的 `ENOENT` 容错，避免 Obsidian 或系统回收站已抢先处理附件时中断清理。
- 扩展 Live Preview 图片点击入口，支持点击渲染图片或 widget 邻近区域打开画廊，同时保留源码模式 Markdown 链接的 Obsidian 默认行为。
- 补充删除目录、批量删除、配置示例、预览点击和恢复事务回归测试，并同步更新用户指南、设置说明、任务状态与网站版本文案。
...
- 补充设置页回归测试，确保日志开关固定显示在诊断日志分组中。
- 同步更新 `manifest.json`、`package.json`、`versions.json`、README、任务状态与网站版本文案，准备以 `4.0.7` 发布。

## 4.0.6

- 新增插件自有的详细调试日志开关，不再依赖 Obsidian 调试模式；开启后控制台会输出命令开始、完成、跳过、失败和关键处理分支日志。
- 命令失败日志会直接输出 `Error` 对象，方便在开发者控制台展开调用栈；恢复命令也纳入统一命令日志。
- 新增恢复命令失败日志回归测试，并把网站 lint 纳入 `npm run validate`。
- 更新用户指南、测试用例和文档索引中的诊断日志说明。
