...
- 状态: `Implemented`
- 目标: 验证详细调试日志由插件设置控制，且命令失败时带调用栈
- 步骤:
  1. 打开开发者控制台，过滤 `Note Image Manager`
  2. 在设置页开启 `启用详细调试日志`
  3. 执行任意插件命令，例如 `压缩图片【单文件】`
  4. 触发一个失败场景，例如让恢复命令在无效恢复数据或手工模拟错误环境中失败
  5. 关闭 `启用详细调试日志`
- 预期:
  - 开启后控制台出现 `[Note Image Manager] Debug logging enabled`
  - 执行命令时出现 `Command started` 与 `Command completed`
  - 命令失败时出现 `Command failed`，并可展开 `Error` 对象查看调用栈
  - 关闭后出现一次 `Debug logging disabled`
  - 关闭后再次执行命令，不再输出详细命令日志
...
- 删除使用 `./assets/${noteFileName}` 的 md 后，附件已由 Obsidian 或插件删除时，插件会继续删除变空的 `assets/<note>` 目录。
- 批量删除多个 md 且图片有交叉引用时，只有元数据 resolved 后才会删除未被其他现存 md 引用的图片；元数据超时只删除真实空目录。
- 删除使用留空、`Attachments/Images` 或 `./assets` 保存位置的 md 时，插件不会自动扫描或删除这些共享目录。
- 点击画廊缩略图时，灯箱大图不为空白。
- 开启详细调试日志时，所有命令都会输出开始 / 完成 / 失败日志；关闭后不再输出详细命令日志。
