...
本文档覆盖 `obsidian-image-manager` 当前已交付与部分交付的功能集合。
...
- Obsidian 桌面版
- 已启用社区插件的测试仓库
- 已将插件构建产物复制到 `.obsidian/plugins/obsidian-image-manager/`
- 已加载并启用当前插件版本
...
- `更新图片链接与目录` 在开启自动下载后，会同步下载 `URL`、`file://`、`data:image/...;base64,...` 图片源。
