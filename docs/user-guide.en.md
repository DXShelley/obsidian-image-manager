...
- `Delete empty image folders` is on by default. After orphan images are deleted, folders that become empty are cleaned up upward. Automatic cleanup after deleting a Markdown note only targets note-owned folders such as `./assets/${noteFileName}` or `./assets/{noteName}`.
- `Delete orphan images` is off by default. When enabled, `Update image links and directories` also removes images in the current managed folder that are not referenced by any Markdown note. Fixed shared folders and sibling shared folders are judged within the current command scope, so verify with single-note or single-folder commands first.
...
- If `outputFolder` is empty, points to a fixed shared library such as `Attachments/Images`, or points to a sibling shared folder such as `./assets`, the plugin does not automatically scan or delete that shared folder when one Markdown note is deleted. Treat those locations as shared pools instead of enabling note-rename sync.
