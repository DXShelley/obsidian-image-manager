...
- 所有带范围的命令会按 `id` 中的 `a1-a5`、`b1-b5`、`c1-c5` 顺序显示，分别对应 `【单文件】`、`【单文件夹】`、`【整库】`，组内再按 `更新链接 -> 下载外部图片 -> 格式转换 -> 压缩 -> 删除多余图片文件` 排序。
- `【单文件】更新图片链接与目录`
- `【单文件】下载外部图片到本地`
- `【单文件】转换图片为默认格式`
- `【单文件】压缩图片`
...
- `下载外部图片到本地` 会把笔记中的 `URL`、`file://` 与 `data:image/...;base64,...` 图片源导入到受管目录，并将链接改写为本地图片链接。
- 命令面板中的 `【单文件】下载外部图片到本地` 会处理当前笔记中的全部外部图片链接。
...
- 批量命令优先按 `【单文件】`、`【单文件夹】` 逐步收敛，确认链接重写和目录迁移符合预期后，再执行 `【整库】` 范围命令。
